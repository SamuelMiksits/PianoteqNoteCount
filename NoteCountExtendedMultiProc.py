...
def calcKeyList(file):
    keyList = initializeList()
    
    try:
        keyList = parseMidiFile(mido.MidiFile(file[0] + "/" + file[1] + "/" + file[2]))
    except:
        try:
            keyList = parseMidiFile(mido.MidiFile(file[0] + "\\" + file[1] + "\\" + file[2]))
        except OSError:
            print("Warning: File '" + file[0] + "/" + file[1] + "/" + file[2] + "' is likely corrupted!"\
                  + " The file has been excluded from the per note breakdown but is still included in total.") 
...
def main():
    ...
    keyList = initializeList() # A list of all midi keys on the keyboard (NOTE: assumes 88 keys), 
                               ...
                               # played 5124 times
    corruptFileWarning = False # Midi parser raises error if corrupt file is found, print file name and warning at the end
    fileList = list() #List of files for the midi parser to go through
        ...
        if "." not in yearList[i]: # Looks for folders, and not items which contain "." (mitigation to ignore this file in search)
            ...
            for j in range(len(monthList[i])):
                ...
                for k in range(len(archivelist)):
                    ...
                    fileList.append([yearList[i], monthList[i][j], archivelist[k]])
    
    #Create a list of keylists for all midi files, and then calculate the sum afterwards, use pooling to run on all cores. 
    subKeyLists = list()        
    with Pool() as pool:
        subKeyLists = pool.map(calcKeyList, fileList)

    for i in range(len(subKeyLists)):
        keyList = addKeyLists(keyList, subKeyLists[i])
